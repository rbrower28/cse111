...
def main():
    ...
    mass = float(input("Enter its mass(g): "))
    periodic_table = make_periodic_table()

    formula_list = parse_formula(formula, periodic_table)
    ...
    while go_again == "y":
        ...
        print(f"Mass: {moles:.5f} moles")
        go_again = input("\nWould you like to change the mass? (y/n): ").lower()

        if go_again == "y":
            mass = float(input("What is the new mass(g)? "))
        else:
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for element in symbol_quantity_list:
        symbol = element[0]
        quantity = element[1]
        mass = periodic_table[symbol][1]
        elem_mass = mass * quantity
        total_mass += elem_mass

    return total_mass

def make_periodic_table():
    """ Returns the entire periodic table. """
